...
*Mehana and Alex Variable identification

First variables for map:
DP03_0014 = Estimate!! employment status own children of the household are under 6 years 
DP03_0016 = Estimate employment status own children of the house hold 6-17 years

Second variable for our scatter plot: 
DP05_0082 = estimate of Hispanic or Latino and race. total population. not Hispanic or Latino. Asian alone

* variables = c(estimate_of_children_over6 = "DP03_0016"),

```{r, message=FALSE, warning=FALSE, results='hide'}
acs_2022_variables <- get_acs(
  geography = "tract",
  variables = c(percent_of_children_under6 = "DP03_0014P", percent_of_children_over6 = "DP03_0016P"), 
  state = "CA",
  county = "Orange",
  geometry = TRUE,
  output = "wide",
...
```{r}
children_california_map <- ggplot(acs_2022_variables) +
  geom_sf_interactive(
    aes(
      fill = percent_of_children_under6E | percent_of_children_over6E, 
      tooltip = paste(NAME, ": ", percent_of_children_under6E | percent_of_children_over6E, "%"),
      data_id = NAME
    )
  ) +
  scale_fill_viridis_d(option = "blue") +
  theme_void() 

girafe(ggobj = children_california_map, width = 800, height = 500)
...
percent_employment_2015_2022 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(percent_of_employed_over_16 = "DP03_0001P"),
    state = "CA",
    county = "Orange",
    year = .x,
    survey = "acs5"
...
```{r}
ggplot(percent_employment_2015_2022, aes(x= year, y = estimate)) + 
  geom_line() + 
  geom_point()
```
...
variable 1 "DP02_0053P" children over 3 in school
variable 2 "DP02_0152P" with computer and internet

```{r, message=FALSE}
school_children_with_digital_access <- get_acs(
  geography = "tract",
  variables = c(percent_of_children_in_school = "DP02_0053P",
                percent_of_households_with_computer_and_internet = "DP02_0152P"),
  state = "CA",
  county = "Orange",
  #geometry = TRUE,
  output = "wide",
...
```{r, warning=FALSE, message=FALSE}
ggplot(school_children_with_digital_access, aes(x = percent_of_children_in_schoolE,
                                             y = percent_of_households_with_computer_and_internetE)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
  theme_minimal()
...
We found that when we pulled certain variables that were labeled percent did not have percent values when graphed. Instead estimate values were labeled and pulled. Hence why there is a very large cluster of data in the lower left conner of the scatterplot data
